...
| `complete` | 用户表示某个任务已完成 | 完成路径 |
| `delete` | 用户表示某个事项记错了、要删除、取消或作废 | 删除/取消路径 |
| `maintenance` | 用户要求修改本目录代码、规则、数据库或文档 | 维护路径 |
| `chat` | 用户提出与任务、日程、完成、数据库维护无关的问题或闲聊 | 聊天路径 |

如果用户请求不属于上述基础意图，但像是可扩展能力，先读取 `extensions/catalog.md`。若目录中有匹配项，再读取对应的 `extensions/{name}.md` 并按该扩展说明处理。
...
2. 如果没有 `item_id`，先运行 `python .\assistant.py query --type task --status active`。
3. 如果能唯一、高置信度匹配，完成该任务。
