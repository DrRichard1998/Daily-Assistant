...
【为避免环境配置问题，推荐优先安装release中的便携包zip】
...
如果要分发给没有安装 Python 的 Windows 用户，可以生成带内置 Python 运行时的便携目录：
...
默认不会复制当前项目的私人数据库，只会在便携目录中初始化一个新的 `data/assistant.sqlite`。如果明确需要把当前数据库一起打包，可运行：
